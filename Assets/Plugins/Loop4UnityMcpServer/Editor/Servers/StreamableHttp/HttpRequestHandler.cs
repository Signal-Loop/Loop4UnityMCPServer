...
namespace LoopMcpServer.Servers.StreamableHttp
{
    ...
    public sealed class HttpRequestHandler
    {
        ...
        private async UniTask HandleGetAsync(HttpListenerContext context, CancellationToken ct)
        {
            ...
            if (!string.IsNullOrEmpty(lastEventId) && LoopMcpServerSettings.Instance.VerboseLogging)
            {
            ...
            try
            {
                ...
                var keepAliveInterval = Math.Max(5, LoopMcpServerSettings.Instance.SseKeepAliveIntervalSeconds);
                try
                {
